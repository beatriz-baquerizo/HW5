...
<<<<<<< HEAD
		...
		while (moved < CAPACITY) {
			// Check if current position is empty
			if (table[currentPos] == null) {
				table[currentPos] = new Bucket<>(currentKey, currentValue);
				return;
			}

			// Check for duplicate <key, value> at current position
			if (table[currentPos].getBucKey().equals(currentKey) && table[currentPos].getValue().equals(currentValue)) {
				return; // already exists, do nothing
			}

			// Kick out existing element and save what's currently in bucket
			K kickedKey = table[currentPos].getBucKey();
			V kickedValue = table[currentPos].getValue();

			// Place current element in this bucket (overwrite)
			table[currentPos] = new Bucket<>(currentKey, currentValue);

			// Prepare to relocate the kicked-out element to its alternate position
			currentKey = kickedKey;
			currentValue = kickedValue;

			// Calculate both positions for kicked element
			int pos1 = hash1(currentKey);
			int pos2 = hash2(currentKey);

			// Move to whichever position it's NOT currently at
			if (currentPos == pos1) {
			   currentPos = pos2;
			} else {
			    currentPos = pos1;
